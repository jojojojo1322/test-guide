import { Box, Typography, Paper, Tabs, Tab, Stack, TextField, Divider, Chip, Button } from "@mui/material";
        ...
        <Stack spacing={2} sx={{ mt: 3 }}>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center">
            <TextField
              ...
              size="small"
              value={query.from}
              onChange={(event) => setQuery({ from: event.target.value }, { replace: true })}
            />
            <TextField
              label="To"
              placeholder="YYYYMMDD"
              size="small"
              value={query.to}
              onChange={(event) => setQuery({ to: event.target.value }, { replace: true })}
            />
            <Button variant="outlined" onClick={() => resetQuery({ replace: true })}>
              Reset
            </Button>
          </Stack>
          ...
          <Typography variant="body2" color="text.secondary">
            URL 쿼리와 화면 상태가 자동으로 동기화됩니다. (예: ?tab=2&from=20251011&to=20251011)
          </Typography>
